...
def create_fallback_queries(search_terms: List[str]) -> List[str]:
    ...
    # Query 1: Find any entity containing first search term
    fallback_queries.append(
        f'MATCH (n:Entity) WHERE toLower(n.name) CONTAINS toLower("{search_terms[0]}") RETURN n.name LIMIT 20'
    )
    
    # Query 2: Find entities and their relationships containing search terms
    if len(search_terms) >= 2:
        fallback_queries.append(
            f'MATCH (n:Entity)-[r]-(m:Entity) WHERE toLower(n.name) CONTAINS toLower("{search_terms[0]}") OR toLower(m.name) CONTAINS toLower("{search_terms[1]}") RETURN n.name, r, m.name LIMIT 20'
        )
    
    # Query 3: Broader search - find any relationships
    fallback_queries.append(
        f'MATCH (n:Entity)-[r]-(m:Entity) WHERE toLower(n.name) CONTAINS toLower("{search_terms[0]}") RETURN n.name, r, m.name LIMIT 20'
    )
    
    # Query 4: Multi-hop search for related entities
    fallback_queries.append(
        f'MATCH (n:Entity)-[*..3]-(m:Entity) WHERE toLower(n.name) CONTAINS toLower("{search_terms[0]}") RETURN DISTINCT n.name, m.name LIMIT 20'
    )
    
    # Query 5: Return all entities to see what's available
    fallback_queries.append(
